// 3. Напишите программу, которая определяет адрес, где находятся целочисленная, действительная, символьная переменные.
// Используя арифметику указателей(!), вычислите размер памяти, занятый каждой такой переменной.
// На экран вывести адреса, по которым расположены переменные, и размер занимаемой памяти в байтах.
// (Использование функции sizeof не соответствует условию задания; для вывода адреса символьной переменной следует использовать преобразование типа к void*)
